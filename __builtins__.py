...
type Any = (
    ...
    | range_class
    | Callable[..., Any]  # Python builtin    - functions / modules
    | _tuple[Any, ...]
    | _list[Any]  # Python builtin    - collection types
    | _set[Hashable]
    | _dict[Hashable, Any]
    | Direction
...
class Entities(_Enum):
    ...
    def _generate_next_value_(
        name: string, start: _int, count: _int, last_values: _list[_Any]
    ) -> Entity:
        return Entity()
...
type SimulateUnlocks = (
    _dict[Unlock, _int]
    | _tuple[_tuple[Unlock, _int]]
    | _list[_tuple[Unlock, _int]]
    | _tuple[Unlock]
    | _list[Unlock]
    | Unlocks
)
...
# -------------------------------------------------------------------------------
def get_cost(
    thing: Entity | Entities | Item | Items | Unlock | Unlocks,
    level: _int | None = None,
) -> _dict[Item, _int] | None:
    """
...
# --------------------------------------------------
def leaderboard_run(
    leaderboard: Leaderboard, file_name: string, speedup: _float
) -> None:
    """
